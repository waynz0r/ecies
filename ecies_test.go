...
func TestDifferentCurve(t *testing.T) {
	curve := elliptic.P256()
	{
		...
		publicKey := k.PublicKey
		ecies := NewECIES(curve)
		assert.Equal(t, GetECPointByteLength(curve), ecies.ecPointByteSize)
		plainText := "hello world"
		encryptedTextBytes, err := ecies.Encrypt(publicKey, []byte(plainText))
		if err != nil {
			t.Error("failed to encrypt message")
...
func TestCompatibilityWithBouncyCastle(t *testing.T) {
	{
		curve := elliptic.P256()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("344b09adcd5048f7f2e83bdabc155587c0071769b86c4715137e8fd06e30f349"))
		ecies := NewECIES(curve)
	...
	{
		curve := elliptic.P384()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("cfd0e0017542dbdee76fb2c0c89230d16cf874c09b54d6f454e9180660aba1b135ea332675054fe3bc7a0aee6ccde68b"))
		ecies := NewECIES(curve)
	...
	{
		curve := elliptic.P521()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("01b0ffc246bf9ae9bf5be2112128be6c19bb86b1bd7925bea2cf2269fda5c092a8d5c9d2353699bab4d2875ab8a5f7e1f0f281d66e2de39b6c0e4e03f2c047575533"))
		ecies := NewECIES(curve)
	...
	{
		curve := elliptic.P256()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("19f209fc65430e95fda48dbe673bc68309535584f65bc594371fc350334074ef"))
		ecies := NewCustomizedECIES(NewEcsvdpDhKeyAgreement(), NewAesCbcPkcs7Cipher(), NewKeyDerivationFunction1(crypto.SHA256), crypto.SHA256, 16, 16, GetECPointByteLength(curve))
	...
	{
		curve := elliptic.P256()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("16559c42d4e9f20d6e4815306edf9c17201fa5d913c38d7c164f186c5207aaa2"))
		ecies := NewCustomizedECIES(NewEcsvdpDhKeyAgreement(), NewAesCbcPkcs7Cipher(), NewKeyDerivationFunction1(crypto.SHA1), crypto.SHA1, 16, 16, GetECPointByteLength(curve))
	...
	{
		curve := elliptic.P256()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("f1e99bae75f88b1bdd24303c3d484bbe85c9dc51382807d16e67e629318e96ea"))
		ecies := NewCustomizedECIES(NewEcsvdpDhKeyAgreement(), NewAesCbcPkcs7Cipher(), NewKeyDerivationFunction1(crypto.SHA384), crypto.SHA384, 16, 16, GetECPointByteLength(curve))
	...
	{
		curve := elliptic.P256()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("6ed14bb52c36ef2b7aae22e65ce08cfd0aaa09b930d3e949c39c3f845795b1bf"))
		ecies := NewCustomizedECIES(NewEcsvdpDhKeyAgreement(), NewAesCbcPkcs7Cipher(), NewKeyDerivationFunction1(crypto.SHA512), crypto.SHA512, 16, 16, GetECPointByteLength(curve))
	...
	{
		curve := elliptic.P256()
		privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError("a80b542432050d3344f173a1aa21c8d3fb1a4abfeb0159f2c472625e6dbc257d"))
		ecies := NewCustomizedECIES(NewEcsvdpDhKeyAgreement(), NewAesGcmCipher(), NewKeyDerivationFunction1(crypto.SHA256), crypto.SHA256, 16, 16, GetECPointByteLength(curve))
...
func TestErrorDecryption(t *testing.T) {
	...
	privateKey := DeserializePrivateKey(curve, HexDecodeWithoutError(privateKeyString))
	ecies := NewECIES(curve)
